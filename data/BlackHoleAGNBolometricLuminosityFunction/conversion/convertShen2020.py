...
raw = np.genfromtxt(input_filename,dtype=None,usecols=(0, 1, 2, 3, 4, 5)) #np.loadtxt(input_filename, delimiter=delimiter, dtype = ('str','float','float','float','float','float'),usecols=(0, 1, 2, 3, 4, 5))
...
L_low = np.array([10 ** (raw[j][2] - raw[j][3]) for j in range(len(raw))]) * unyt.erg / unyt.s
L_high = np.array([10 ** (raw[j][2] + raw[j][3]) for j in range(len(raw))])* unyt.erg / unyt.s

Phi = np.array([10**raw[j][4] for j in range(len(raw))]) / unyt.Mpc ** 3
Phi_low = np.array([10 ** (raw[j][4] - raw[j][5]) for j in range(len(raw))]) / unyt.Mpc ** 3
Phi_high = np.array([10 ** (raw[j][4] + raw[j][5]) for j in range(len(raw))]) / unyt.Mpc ** 3
...
citations = ["Shen+ (2020), hard_X_rays", "Shen+ (2020), soft_X_rays", "Shen+ (2020), B_band", "Shen+ (2020), UV", "Shen+ (2020), mid_IR"]
...
for i in range(np.size(wavelength_band_types)):
    comment = (
        ...
        " 2020ApJ.892.17D. This file contains data in the " + wavelength_band_types[i] + " band."
    )
    ...
    for z in redshift_list:
        ...
        processed.associate_x(
            ...
            scatter=np.array([x_scatter[0][(wavelength_band==wavelength_band_types[i])&(redshifts==z)], x_scatter[1][(wavelength_band==wavelength_band_types[i])&(redshifts==z)]]) * unyt.erg / unyt.s,
            comoving=False,
        ...
        processed.associate_y(
            ...
            scatter=np.array([y_scatter[0][(wavelength_band==wavelength_band_types[i])&(redshifts==z)], y_scatter[1][(wavelength_band==wavelength_band_types[i])&(redshifts==z)]]) / unyt.Mpc ** 3,
            comoving=False,
    ...
    output_path = f"{output_directory}/"+output_filename_base+wavelength_band_types[i]+"_Data.hdf5"
