...
Phi = (1e-4 * 0.7**3 * raw[:, 3]) * unyt.dimensionless
Phi_low = (1e-4 * 0.7**3 *(raw[:, 3]-raw[:, 5])) * unyt.dimensionless
Phi_high = (1e-4 * 0.7**3 *(raw[:, 3]+raw[:, 4])) * unyt.dimensionless

# Define the scatter as offset from the mean value
x_scatter = unyt.unyt_array((M_BH - M_BH_low, M_BH_high - M_BH))
y_scatter = unyt.unyt_array((Phi - Phi_low, Phi_high - Phi))

comment = (" The black hole mass function estimate taken from Vika et al. (2009):"
           " 2009MNRAS.400.1451V."
